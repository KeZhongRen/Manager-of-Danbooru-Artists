...
<img width="786" height="200" alt="46de252f945ce377b0071dbddcd8ee0e" src="https://github.com/user-attachments/assets/5c0d6d2c-89ac-4f9e-80bc-636870166a99" />

建议用一个专门的文件夹存放exe文件，后续会在同目录生成数据库文件夹的导向文件。并且默认数据库的位置与exe文件同目录（数据库的目录可修改）。

It is recommended to use a dedicated folder to store exe files. Later, a shortcut file for the database folder will be created in the same directory. By default, the database is located in the same directory as the exe file (the database directory can be modified).
...
在上方，设置界面，选择图片数据的来源文件夹，其中png文件的名称应该为(.+)_\d{5}_\.png（前面为画师名），以及选择本程序的资源储存目录，还有ui界面的调节也在这个面板。

At the top, the settings interface, select the source folder of the image data, where the name of the png file should be (.+)_d{5}_.png, and select the resource storage directory of this program (it is recommended to create a separate folder), and the adjustment of the UI interface is also in this panel.
